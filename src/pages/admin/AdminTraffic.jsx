// ─────────────────────────────────────────────────────────────
// pages/admin/AdminTraffic.jsx
// Detailed visitor logs and geolocation statistics.
// Allows staff to view real-time site visit traffic, page views, and origin locations.
...
import {
  FiEye, FiTrash2, FiRefreshCw, FiSearch, FiClock,
  FiGlobe, FiMonitor, FiSmartphone, FiCalendar, FiFilter, FiMapPin
} from 'react-icons/fi'
import toast from 'react-hot-toast'
import { getTrafficLogs, deleteTrafficLog } from '../../services/traffic'
...
export default function AdminTraffic() {
  ...
  // Real-time site traffic counter states from Firestore documents
  ...
  // Fetch both stats documents and raw traffic logs
  const fetchData = async () => {
    setLoading(true)
    try {
      // 1. Fetch Logs
      const trafficLogs = await getTrafficLogs()
      setLogs(trafficLogs)
      ...
      const [totalDoc, dailyDoc, monthlyDoc] = await Promise.all([
        getDoc(doc(db, 'siteStats', 'total')),
        getDoc(doc(db, 'siteStats', `daily_${dayKey}`)),
        getDoc(doc(db, 'siteStats', `monthly_${monthKey}`))
      ])
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this traffic log?')) return
    setDeletingId(id)
    try {
      await deleteTrafficLog(id)
      setLogs((prev) => prev.filter((log) => log.id !== id))
      toast.success('Traffic log deleted successfully')
    } catch {
      toast.error('Failed to delete traffic log')
    } finally {
      setDeletingId(null)
    }
  }

  // Filter logs based on search term (matches IP, City, Region, Country, Page, Browser)
  ...
  // Get list of unique pages logged for page filter dropdown
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy-800 flex items-center gap-2">
            <FiGlobe className="text-primary-600 animate-pulse" />
            Visitor Traffic Logs
          </h1>
          <p className="text-gray-500 text-sm mt-0.5">
            Monitor real-time site visits, page views, and approximate geolocation mappings.
          </p>
        </div>
        <button
          onClick={fetchData}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Today Card */}
        <div className="card p-5 border-l-4 border-blue-500 bg-white hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Today's Visits</p>
        ...
        <div className="card p-5 border-l-4 border-emerald-500 bg-white hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">All-Time Page Views</p>
      ...
      <div className="card p-4 bg-white flex flex-col sm:flex-row items-center gap-4">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Search by IP, city, state, country, or browser..."
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="card flex flex-col items-center justify-center py-16 text-gray-400 bg-white border border-gray-100 rounded-xl">
          <FiGlobe size={40} className="mb-3 opacity-30" />
          <p className="font-medium text-gray-500">No traffic logs found</p>
          <p className="text-xs text-gray-400 mt-1">Try resetting the filters or verify site visits.</p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex items-start gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-primary-50 text-primary-700">
                          <FiMapPin size={10} />
                          {locationText}
                        </span>
                      </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 pt-3 sm:pt-0 border-gray-50 shrink-0">
                    <div className="text-left sm:text-right">
                      ...
                      <p className="text-2xs text-gray-400 mt-0.5">Session Started</p>
                    </div>
                    {isAdmin() && (
                      <button
